...
我们的形状属性集预测遵循经典的前馈集预测网络DETR【7】的架构。具体而言，图像编码器提取的图像特征与可学习位置编码连接后作为Transformer编码器的输入。接着，Transformer解码器将 mmm 个可学习查询作为输入，并将其转换为输出嵌入，即 z∈Rm×dz \in R^{m \times d}z∈Rm×d。输出嵌入随后通过三个预测头并行解码，即基元类型预测头、变换预测头和颜色预测头。

- **基元类型预测**：对于Sec. 3.1中定义的基元集合，每个变换（在 mmm 个之中）有 nnn 种可能的选择。因此，基元类型预测头通过softmax将嵌入映射为一个 nnn-维的概率分布，即 Fw:z→W∈Rm×nF_w : z \to W \in R^{m \times n}Fw​:z→W∈Rm×n，从而将 AAA 放松为概率分布。

- **变换预测**：为了重构/近似各种复杂形状，我们定义了一个仿射变换矩阵 MMM，按旋转-缩放-平移-错切顺序对基元进行变换，包含旋转因子 α\alphaα、缩放因子 (sx,sy)(s_x, s_y)(sx​,sy​)、平移因子 (tx,ty)(t_x, t_y)(tx​,ty​) 和错切因子 (βx,βy)(\beta_x, \beta_y)(βx​,βy​)。因此，仿射变换预测可以定义为映射：Fs:z→θ∈Rm×7F_s : z \to \theta \in R^{m \times 7}Fs​:z→θ∈Rm×7。为了使变换后的基元更精确地贴合形状，我们在仿射变换上引入点状偏移 Δ∈Rm×2t\Delta \in R^{m \times 2t}Δ∈Rm×2t 作为细化，其中 ttt 为每个基元的参数点数。因此，每个变换 TiT_iTi​ 可以看作 M(θi)M (\theta_i)M(θi​) 和 Δi\Delta_iΔi​ 的组合。

- **颜色预测**：同样地，我们将每个嵌入映射到颜色：Fc:z→C∈Rm×3F_c : z \to C \in R^{m \times 3}Fc​:z→C∈Rm×3。
