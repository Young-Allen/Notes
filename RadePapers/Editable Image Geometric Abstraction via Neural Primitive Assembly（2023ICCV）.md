...
- **变换预测**：为了重构/近似各种复杂形状，我们定义了一个仿射变换矩阵 M，按旋转-缩放-平移-错切顺序对基元进行变换，包含旋转因子 $\alpha$ 、缩放因子 $(s_x, s_y)$ 、平移因子 $(t_x, t_y)$ 和错切因子$(\beta_x, \beta_y)$ 。因此，仿射变换预测可以定义为映射：$F_s : z \to \theta \in R^{m \times 7}$ 。为了使变换后的基元更精确地贴合形状，我们在仿射变换上引入点状偏移 $\Delta \in R^{m \times 2t}$ 作为细化，其中 ttt 为每个基元的参数点数。因此，每个变换 $T_i$​ 可以看作 $M (\theta_i)$ 和 $\Delta_i$ 的组合。
